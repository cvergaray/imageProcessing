...
public class HoughTransform extends Thread { 
    ...
    public static void main(String[] args) throws Exception { 
        ...
        double rAccum = 0;
        double count = 0;
        int max = lines.size() > 10 ? 10 : lines.size();
        // draw the top 10 lines back onto the image 
        for (int j = 0; j < max; j++) { 
            HoughLine line = lines.elementAt(j); 
            line.draw(image, Color.RED.getRGB()); 
            System.out.println("Theta: " + line.theta + " Distance: " + line.r);
            thetaAccum += line.theta;
            rAccum += line.r;
            count++;
        } 
        ...
        HoughLine line = new HoughLine(thetaAccum, rAccum);
        line.draw(image, Color.CYAN.getRGB());
        ...
        for (int x = 0; x < image.getWidth(); x++) { 
            for (int y = 0; y < image.getHeight() - 1; y++) { 
                ...
                if (((image.getRGB(x, y) & 0x000000ff) == 0) &&
                     (image.getRGB(x, y+1) & 0x000000ff) != 0) { 
                    addPoint(x, y); 
                } 
            } 
        } 
